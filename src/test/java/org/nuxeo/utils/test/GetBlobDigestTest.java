...
import org.nuxeo.utils.GetBlobDigest;
...
public class GetBlobDigestTest {
    ...
    protected String testOneHash(String hashKind) throws Exception {
        ...
        _blob = new StringBlob( kSTRING, "text/plain", "UTF-8" );
        _blob.setFilename("thefile.txt");

        OperationContext ctx = new OperationContext(session);
        ctx.setInput(_blob);

        OperationChain chain = new OperationChain("testChain_" + hashKind);
        chain.add(GetBlobDigest.ID).set("digestKind", hashKind)
                                     .set("contextVarName", "theVar");

        service.run(ctx, chain);
        return (String) ctx.get("theVar");
    }
